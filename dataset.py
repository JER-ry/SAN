...
class HYBTr_Dataset(Dataset):
    ...
    def collate_fn(self, batch_images):
        ...
        max_length = max(item[1].shape[0] for item in batch_images)
        ...
        labels = torch.zeros((len(batch_images), max_length, 11)).long()
        labels_masks = torch.zeros((len(batch_images), max_length, 2))
        ...
        for i in range(len(batch_images)):
            ...
            l = batch_images[i][1].shape[0]
            labels[i][:l, :] = batch_images[i][1]
            labels_masks[i][:l, 0] = 1

            for j in range(batch_images[i][1].shape[0]):
                labels_masks[i][j][1] = batch_images[i][1][j][4:].sum() != 0

        images = torch.nested_tensor(
            [i[0] for i in batch_images]
        ).to_padded_tensor(0)
        image_masks = torch.nested_tensor(
            [torch.ones_like(i[0]) for i in batch_images]
        ).to_padded_tensor(0)
