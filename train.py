...
parser.add_argument(
    "--config", default="copy.yaml", type=str, help="path to config file"
)
...
model.name = f'{params["experiment"]}_{now}'
print(model.name)
...
min_step = 0
for epoch in range(
    params["epoch_offset_for_lr"], params["epoches"] + params["epoch_offset_for_lr"]
):
    ...
    # log_metric("L", eval_loss)
    # log_metric("W", eval_word_score)
    # log_metric("S", eval_node_score)
    # log_metric("E", eval_expRate)

    print(
        f"Ep_{epoch+1} L_{eval_loss:.5f} W_{eval_word_score:.5f} S_{eval_node_score:.5f} E_{eval_expRate:.5f}"
    )
